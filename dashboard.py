...
def obter_dados_do_banco(categoria, subcategorias, ano_inicio, ano_fim):
    try:
        ...
        for subcat in subcategorias:
            nome_tabela = f"{categoria}_{subcat}"
            
            # Query para o mapa
            query_mapa = f"""
                SELECT ie.ano, ie.chave as pais, ie.valor, pais.latitude, pais.longitude 
                FROM {nome_tabela} ie 
                JOIN paises as pais ON ie.chave = pais.pais
                WHERE CAST(ie.ano AS INTEGER) BETWEEN %s AND %s
                AND pais.pais <> 'Total'
            """
            df_mapa = pd.read_sql_query(query_mapa, conn, params=(ano_inicio, ano_fim))
...
def painel_dashboard():
    ...
    categoria = st.sidebar.radio('Categoria', ['Exportação', 'Importação'])
    ...
    with col1:  
        ...
        st.write(df_mapa.drop(columns=['latitude', 'longitude']))
    
    with col2:
        # Painel do Gráfico de Barras Tipo Pareto
        st.subheader('Gráfico de Pareto')
        grafico_buf = criar_grafico_barras(df_mapa)
        if grafico_buf:
